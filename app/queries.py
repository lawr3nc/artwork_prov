...
location = """SELECT DISTINCT ?x ?y
                WHERE
                    {
                        {?x art:location ?y.} UNION {?x gallery:location ?y.} UNION {?x transport:destination ?y.}
                    }"""

specialist = """SELECT DISTINCT ?w ?x ?y ?z
                WHERE
                    {
                        ?w auction:specialistname ?x.
                        ?w auction:specialistdob ?y.
                        ?w auction:specialistgender ?z.
                    }"""

hammerprice = """SELECT DISTINCT ?x ?y
                WHERE
                    {
                        ?x auction:hammerprice ?y.
                    }"""

auctionhouse = """SELECT DISTINCT ?x ?y
                WHERE
                    {
                        ?x auction:auctionhouse ?y.
                    }"""

gallery = """SELECT DISTINCT ?x ?y ?z
                WHERE
                    {
                        ?x gallery:name ?y.
                        ?x gallery:owner ?z.
...
buyer = """SELECT DISTINCT ?w ?x ?y ?z
                WHERE
                    {
                        ?w auction:purchasername ?x.
                        ?w auction:purchaserdob ?y.
...
def runQuery():
    ...
    if form.validate_on_submit():
        ...
        if prov_docdb is None:
            ...
            provn_doc = api.document.get(prov_docdb.docid).prov
    ...
    return render_template('query.html', title='Queries', form=form)
